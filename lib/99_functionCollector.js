...
export function invokeMoveMachine(oldCanvasId, newCanvasId) {
    let machineId, machineNr, machineToMove, newPlace, oldPlace, result, machineCount, jsonObject, spaceInNewBay, user,
        newMachineCount, oldMachineCount, ecnFirstMachine, ecnSecondMachine;
    user = Meteor.user().username;
    try {
        newPlace = activeAssembly.findOne({_id: newCanvasId});
        oldPlace = activeAssembly.findOne({_id: oldCanvasId});
        // check space in new canvas and how many machines are there already
        spaceInNewBay = newPlace.baySpace;
        newMachineCount = newPlace.bayArray.length;  // count machines in new Bay
        // check how many Machines in old Bay
        oldMachineCount = oldPlace.bayArray.length;
        ecnFirstMachine = oldPlace.bayArray[0].ecnMachine;
        ecnSecondMachine = oldPlace.bayArray[1].ecnMachine;
        //  console.log('Move machines ', oldMachineCount, newMachineCount)
        // console.log('test 1 ', oldCanvasId, newCanvasId, spaceResult)
        // identify machineId, machineNr
        machineId = oldPlace.bayArray[0].machineId;
        machineNr = oldPlace.bayArray[0].machineNr;
...
export function invokeDrawTwoMachines(presentMachine, newMachine, newCanvasId, presentMachineStatus, newMachineStatus) {
    Meteor.defer(function() {
