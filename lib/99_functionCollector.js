...
export function timeCounter(bayId, timerId) {
    ...
    let  leavingDateTime, moveTime, result, machineNr, timeLine, dateToday, leavingTime, trueMovingTime,
        h, m, s;
    try {
        result = activeAssembly.findOne({_id: bayId});
        machineNr = result.bayArray[0].machineNr;timeLine = machineCommTable.findOne(
            {machineId: machineNr},{fields: {timeLine: 1}})
        leavingTime =
        leavingDateTime = timeLine.timeLine.bay4 + ' ' + leavingTime;
    //    leavingDateTime = Session.get('bayLeaving') // present Machine In Bay Date - Time Leaving Time
        dateToday =  Date.now();
        moveTime =  ((leavingDateTime - dateToday) / 1000).toFixed(0)
