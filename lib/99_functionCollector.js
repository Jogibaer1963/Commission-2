...
export function unitCounter(bayId, bayIdTime) {
 ...
 //   console.log('return value ', bayId,  returnValue)
    return  returnValue
}
...
export function timeCounter(bayId, bayIdTime, timerId) {
  //  console.log(bayId, bayIdTime, timerId)
   ...
   // console.log('time counter', bayId, bayIdTime, timerId)
    try {
        ...
        if (unitCounter === undefined) {
            ...
            firstSlice = unitCounter.slice(0,1);
            secondSlice = parseInt(unitCounter.slice(1, 4)) // takes 2 digits in case line is more than 9 units behind
        ...
        if (moveTime < 0) {
           ...
           // console.log('move time ', negativeMoveTime, 'Bay Leaving', bayLeaving, trueMovingTime)
        }
    } catch(err) {}
