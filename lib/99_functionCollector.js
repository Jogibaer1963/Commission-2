...
//  *********************************    Calctime for Assembly line Overview / Statistics   ***************
...
export function  invokeDrawMachineInBay(canvasId) {
   ...
   //console.log('invoke draw 1 machine ', result, canvasId)
    try {
        if (result.bayArray.length === 0) {
            ...
            let ecnMachine = result.timeLine.ecnMachine;
          //  console.log('ecn invoke 1 machine ', ecnMachine)
            invokeDrawOneMachine(machineNrInBay, canvasId, ecnMachine);
            ...
            //Calls function to draw two machines2002
...
export function invokeMoveMachine(oldCanvasId, newCanvasId) {
    let machineId, machineNr, machineToMove, result, machineCount, jsonObject, spaceResult, space_in_Bay, user,
        oldCanvasCheck, newCanvasCheck, ecnCheck, ecnResult_old, ecnMachine;
    user = Meteor.user().username;
    oldCanvasCheck = activeAssembly.findOne({_id: oldCanvasId}) //check machines in old Canvas
    newCanvasCheck = activeAssembly.findOne({_id: newCanvasId}) //check machines in new Canvas
    if (oldCanvasCheck.bayArray[0].ecnMachine === undefined) {
        ecnResult_old = '0'
    } else {
        ecnResult_old = 'x'
    }

    if (newCanvasCheck.bayArray.length === 2) {
        // 2 Machines detected, break function
        Bert.alert('3 Machines in one Bay is not possible', 'danger', 'growl-top-left')
        return [0]
    }
    console.log('old Canvas ', oldCanvasCheck, "New Canvas ", newCanvasCheck)
    machineNr = oldCanvasCheck.bayArray[0].machineNr

    // check how much space in bay upfront
    if(newCanvasCheck.baySpace === 1) {
        // only space for 1 machine and 1 machine is
    }


    if (oldCanvasCheck.bayArray.length === 2) {
        // 2 Machines in Old Bay, force to move 1st Machine
        console.log('2 machines in old canvas')
        } else if (oldCanvasCheck.bayArray.length === 1) {
            // 1 machine detected
            console.log('1 Machine in old Bay')
        } else if (oldCanvasCheck.bayArray.length === 0) {
            // empty bay, nothing to do
            // console.log('nothing to move')
        } else {
            // no requirements fulfilled, do nothing
        }
    if (newCanvasCheck.bayArray.length === 2) {
        // 2 Machines in Bay upfront, do nothing and throw error message
        console.log('2 machines in Bay upfront')
        } else if (newCanvasCheck.bayArray.length === 1) {
            // 1 Machine in Bay upfront, move Machine in second spot
            console.log('1 machine in Bay upfront')
        } else if (newCanvasCheck.bayArray.length === 0) {
            // Bay is empty, easy move
            console.log('Bay in front is empty')
            invokeDrawOneMachine(machineNr, newCanvasId, ecnResult_old)
            Meteor.call('moveMachineToNextBay', machineNr, user, oldCanvasId, newCanvasId, true, ecnResult_old)
            invokeEmptyBay(oldCanvasId)
        } else {
            // no requirements fulfilled, do nothing
        }
        console.log()
    ...
    if (result[0] === 0) {  // no machine in front
        ...
        if (machineCount === 1) {
            ...
            Meteor.call('moveMachineToNextBay', machineId, machineNr, user, oldCanvasId, newCanvasId, false, ecnResult_old)
            ...
            Meteor.call('moveMachineToNextBay', machineId, machineNr, user, oldCanvasId, newCanvasId, true, ecnResult_old)
        }
        ...
        Meteor.call('moveMachineToNextBay', machineId, machineNr, user, oldCanvasId, newCanvasId, false, ecnResult_old)
        console.log('drawing 2 machines', presentMachineNr, machineNr, newCanvasId)
...
export function invokeDrawTwoMachines(firstMachine, secondMachine, ecnCheckOne, ecnCheckTwo, canvasId) {
    console.log(firstMachine, secondMachine, 'Canvas ID ', canvasId)
    Meteor.defer(function() {
 ...
 export function invokeDrawOneMachine(presentMachine, canvasId, ecnResult ) {

       // console.log('Draw Machine in Bay', presentMachine, canvasId, ecnResult)
        let canvas = document.getElementById(canvasId);
       // console.log('canvas single machine ', canvas)
        let ctx = canvas.getContext("2d");
        ...
        if (canvasId === 'machine_field_front_test_bay' ||
            ...
            if(ecnResult === 'x'){
               // console.log('ecn Result ', ecnResult)
                //If status has something in it, change machine # text to yellow
                ctx.fillStyle = '#DEB511FF'
