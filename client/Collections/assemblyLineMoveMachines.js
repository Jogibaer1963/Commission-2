...
/*
Template.burstSunTimerTeam_3.onRendered(function() {
    let current_theme = 0;
    let counter = 0;
    let sun_team = '.sun-team-3'
    Session.set('percent', 0)
    set_theme(current_theme);
  //  update_clock();
    setInterval(function () {
    ...
    build_sun_spots(10, sun_team);
    set_sun_size(sun_team);
});
...
function build_sun_spots (count, sun_team) {
    let accounted, i, j, ref, results, sun;
    sun = $(sun_team);
    accounted = 0;
    results = [];
...
function set_sun_size (sun_team) {
    let sunHeight, sunWidth;
    sunWidth =  150;
    sunHeight = sunWidth / 3;
    console.log(sun_team)
    return $(sun_team).css({
        top: -1 * (sunHeight / 2),
        left: -1 * (sunWidth / 2),
        width: sunWidth,
